...
module.exports = {
  async getNonLocalizedAttributes(ctx) {
    ...
    const { READ_ACTION, CREATE_ACTION } = siapi.admin.services.constants;
    ...
    const entity = await siapi.query(model).findOne(params);

    if (!entity) {
      return ctx.notFound();
    }

    const permissions = await siapi.admin.services.permission.find({
