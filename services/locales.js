...
const find = (...args) => siapi.query('locale', 'i18n').find(...args);

const findById = id => siapi.query('locale', 'i18n').findOne({ id });

const findByCode = code => siapi.query('locale', 'i18n').findOne({ code });

const count = params => siapi.query('locale', 'i18n').count(params);

const create = async locale => {
  const result = await siapi.query('locale', 'i18n').create(locale);
...
const update = async (params, updates) => {
  const result = await siapi.query('locale', 'i18n').update(params, updates);
...
const deleteFn = async ({ id }) => {
  const localeToDelete = await siapi.query('locale', 'i18n').findOne({ id });

  if (localeToDelete) {
    await deleteAllLocalizedEntriesFor({ locale: localeToDelete.code });
    const result = await siapi.query('locale', 'i18n').delete({ id });
...
const initDefaultLocale = async () => {
  const existingLocalesNb = await siapi.query('locale', 'i18n').count();
...
const deleteAllLocalizedEntriesFor = async ({ locale }) => {
  ...
  const localizedModels = Object.values(siapi.contentTypes).filter(isLocalizedContentType);

  for (const model of localizedModels) {
    await siapi.query(model.uid).delete({ locale }, { returning: false });
  }
};
