...
const shouldApplyLocalesPropertyToSubject = ({ property, subject }) => {
  if (property === 'locales') {
    const model = siapi.getModel(subject);
...
const addAllLocalesToPermissions = async permissions => {
  const { actionProvider } = siapi.admin.services.permission;
...
const syncSuperAdminPermissionsWithLocales = async () => {
  const roleService = siapi.admin.services.role;
  const permissionService = siapi.admin.services.permission;
...
const registerI18nActionsHooks = () => {
  const { actionProvider } = siapi.admin.services.permission;
  const { hooks } = siapi.admin.services.role;
