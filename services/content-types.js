...
const getAndValidateRelatedEntity = async (relatedEntityId, model, locale) => {
  const { kind } = siapi.getModel(model);
  let relatedEntity;

  if (kind === 'singleType') {
    relatedEntity = await siapi.query(model).findOne({});
  } else if (relatedEntityId) {
    relatedEntity = await siapi.query(model).findOne({ id: relatedEntityId });
  }
    ...
    if (attr.type === 'dynamiczone' && isArray(value)) {
      value.forEach(compo => {
        if (has('__component', compo)) {
          const model = siapi.components[compo.__component];
          removeIdsMut(model, compo);
        }
      });
    } else if (attr.type === 'component') {
      const [model] = siapi.db.getModelsByAttribute(attr);
      if (isArray(value)) {
