...
const { contentTypes, parseMultipartData, sanitizeEntity } = require('siapi-utils');
...
const createLocalizationHandler = contentType => {
  ...
  const createFromBaseEntry = async (ctx, entry) => {
    ...
    if (!has('locale', data)) {
      throw siapi.errors.badRequest('locale.missing');
    }

    const matchingLocale = await findByCode(data.locale);
    if (!matchingLocale) {
      throw siapi.errors.badRequest('locale.invalid');
    }

    const usedLocales = getAllLocales(entry);
    if (usedLocales.includes(data.locale)) {
      throw siapi.errors.badRequest('locale.already.used');
    }
    ...
    const newEntry = await siapi.entityService.create(
      { data: sanitizedData, files: sanitizedFiles },
      { model: contentType.uid }
    );

    ctx.body = sanitizeEntity(newEntry, { model: siapi.getModel(contentType.uid) });
  };

  if (isSingleType(contentType)) {
    return async function(ctx) {
      const entry = await siapi.query(contentType.uid).findOne();

      if (!entry) {
        throw siapi.errors.notFound('baseEntryId.invalid');
      }
  ...
  return async function(ctx) {
    ...
    const entry = await siapi.query(contentType.uid).findOne({ id: baseEntryId });
...
const addCreateLocalizationAction = contentType => {
  ...
  siapi.config.routes.push(localizationRoute);

  _.set(siapi, coreApiControllerPath, handler);
};
...
const addGraphqlSchema = schema => {
  _.mergeWith(siapi.plugins.i18n.config.schema.graphql, schema, mergeCustomizer);
};
...
const addGraphqlLocalizationAction = contentType => {
  ...
  if (!siapi.plugins.graphql) {
    return;
  }

  const { toSingular, toPlural } = siapi.plugins.graphql.services.naming;
